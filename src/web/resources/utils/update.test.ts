...
test("shipped branding uses only the current community emblem", () => {
  ...
  assert.match(fullMakefile, /LUCI_MAINTAINER:=jxstarthxr/);
  assert.match(fullMakefile, /LUCI_URL:=https:\/\/github\.com\/jxstarthxr\/luci-theme-fortiwifi-30e/);
  assert.match(liteMakefile, /PKGARCH:=all/);
  assert.match(liteMakefile, /PKG_MAINTAINER:=jxstarthxr/);
  assert.match(liteMakefile, /URL:=https:\/\/github\.com\/jxstarthxr\/luci-theme-fortiwifi-30e/);
